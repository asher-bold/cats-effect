...
object Resources extends IOApp.Simple {
   ...
   */
  def getResourceFromFile(path: String) = Resource.make(openFileScanner(path))(s => IO(s.close()))

  def resourceReadFile(path: String) = getResourceFromFile(path).use(readLineByLine)
